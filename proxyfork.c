...
//static struct flock lock_it, unlock_it; /* helper datastructures for lock and unlock function */
...
int main(int argc, char **argv)
{
    ...
    while (1) {
        ...
        if (pid == 0) {
            ...
            int retval;
            if ((retval = flock(fileno(log_file), LOCK_EX)) < 0) {
                err_sys("Error! Couldn't lock file!");
            }
            ...
            if ((retval = flock(fileno(log_file), LOCK_UN)) < 0) {
                err_sys("Error! Couldn't lock file!");
            }
            //my_lock_release();
